...
9. Follows the exact schema above`;
    try {
        ...
        try {
            const imageBlob = yield hf.textToImage({
            ...
            const buffer = Buffer.from(yield imageBlob.arrayBuffer());
            // Upload to Vercel Blob
            const { url } = yield (0, blob_1.put)(`course-thumbnails/${Date.now()}-${topic.toLowerCase().replace(/\s+/g, '-')}.png`, buffer, { access: 'public' });
